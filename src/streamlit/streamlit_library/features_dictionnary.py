...
# départements
df_dep = pd.read_csv(path_to_dep)
dict_dep = df_dep[['DEP', 'LIBELLE']].to_dict(orient='records')
deps = {dep['DEP']: dep['LIBELLE'] for dep in dict_dep}
deps['201'] = deps['2A']
deps['202'] = deps['2B']
for key in deps:
...
# communes
df_com = pd.read_csv(path_to_com)
dict_com = df_com[['COM', 'LIBELLE']].to_dict(orient='records')
coms = {com['COM']: com['LIBELLE'] for com in dict_com}
for key in coms:
    coms[key] = key + " - " + coms[key]
